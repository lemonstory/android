...
public class SwitchButton extends View {
	...
	private void initView(Context context) {
		...
		mAnimMove = (int) (VELOCITY * density + 0.5f) / 100; //自动滚动速度
	...
	protected void onDraw(Canvas canvas) {
		...
		if(isChecked){
			...
			mPaint.setColor(mSelectCirlceColor);
			canvas.drawText("On", mWidth/2- mTextPaint.measureText("On", 0, 1)*2-2, mHeight/2 + (mTextPaint.measureText("On", 0, 1)/2), mTextPaint);
			canvas.drawCircle(mCriclePostion, mHeight/2, mHeight/2 -5, mPaint);
		}else{
			mPaint.setStyle(Paint.Style.FILL);
			/*mPaint.setColor(mBorderColor);
			canvas.drawRoundRect(rect, mHeight/2, mHeight/2, mPaint);*/
			//RectF innerRect = new RectF(2, 2, mWidth, mHeight -2);
			mPaint.setColor(mUnSelectBg);
			canvas.drawRoundRect(rect, mHeight/2, mHeight/2 , mPaint);
			mPaint.setColor(mUnSelectCircleColor);
			canvas.drawText("Off", mWidth/2-2, mHeight/2 +(mTextPaint.measureText("On", 0, 1)/2), mTextPaint);
			canvas.drawCircle(mCriclePostion, mHeight/2, mHeight/2 -5, mPaint);
		}
	}
	...
	class AnimRunnable implements Runnable{
		...
		private void stopView(boolean endChecked) {
			...
			if(mOnCheckChangeListener != null){
				mOnCheckChangeListener.onCheckedChanged(SwitchButton.this, isChecked);
			}
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		mTextPaint.setTextSize(mWidth/4);
		mStartCriclePos = mHeight / 2;
		mEndCirclePos = mWidth - mHeight /2 -1;
		mCriclePostion = isChecked ? mEndCirclePos :mStartCriclePos; 
