...
public class ShareDialog implements View.OnClickListener{
    ...
    private void setShareParams() {
        ...
        // 设置分享内容
        mShareAction.withText(mShareBean.getText());
        mShareAction.withTitle(mShareBean.getTitle());
        // 设置分享图片, 参数2为图片的url地址
        if(mShareBean.getIconUrl() == null) {
            ...
            mShareAction.withMedia(image);
            mShareAction.withTargetUrl(mShareBean.getUrl());
        }
        //测试新浪微博不能分享音频
        if (mShareAction.getPlatform() != null && !mShareAction.getPlatform().toString().equals("SINA")) {
