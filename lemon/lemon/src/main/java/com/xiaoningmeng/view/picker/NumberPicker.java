...
public class NumberPicker extends LinearLayout {
		...
		if (mDisplayedValues == null) {
			...
			for (int i = 0; i < valueCount; i++) {
				final float textWidth = mSelectorWheelPaint
						.measureText(mDisplayedValues.get(i));
	...
	private void ensureCachedScrollSelectorValue(int selectorIndex) {
		...
		if (selectorIndex < mMinValue || selectorIndex > mMaxValue) {
			...
			if (mDisplayedValues != null) {
				...
				if (mDisplayedValues.size() > displayedValueIndex)
					scrollSelectorValue = mDisplayedValues
							.get(displayedValueIndex);
			} else {
	...
	private boolean updateInputTextView() {
		...
		String text = (mDisplayedValues == null) ? formatNumber(mValue)
				: (mDisplayedValues.size() > mValue - mMinValue) ? mDisplayedValues
						.get(mValue - mMinValue) : "";
