...
public class LHttpRequest {
    ...
    //获取作者专辑列表
    public void getAuthorAlbumsReq(Context context, int authorId, int startAlbumId, int len, String uid, JsonCallback<AuthorAlbums> handler) {

        get(ConstantURL.AUTHOR_ALBUM_LIST).tag(context)
                .addParams("author_id", Integer.toString(authorId))
                .addParams("start_album_id", Integer.toString(startAlbumId))
                .addParams("len", Integer.toString(len))
                .addParams("uid", uid)
                .build().execute(handler);
    }
